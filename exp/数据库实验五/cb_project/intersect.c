...
// 直接利用 tpmms 已经排好序的数据库进行交运算
void intersect(Buffer * buf){
    printf("\n--------------------------------\n");
    printf(RED"基于排序的集合的交算法\n"WHITE);
    printf("--------------------------------\n");
    if (!initBuffer(520, 64, buf)){
        perror("Buffer Initialization Failed!\n");
        return;
    }
    int out_disk_num = INTERSECT_DISK_NUM;
    // 满足关系的元组数
    int cnt = 0;
    ...
    while(r_now < r_end && s_now < s_end){
        if(XY_equal(r_blk + r_index, s_blk + s_index)){
            if(cnt && cnt % 7 == 0){
            ...
            cnt++;
            printf("(X = %d,Y = %d)\n",getX(r_blk + r_index),getY(r_blk + r_index));
            // s 后移
            if(s_index == 48){
                s_now = getAddr(s_blk);
        ...
        // r 后移
        else if(XY_less(r_blk + r_index, s_blk + s_index)){
            if(r_index == 48){
                r_now = getAddr(r_blk);
                freeBlockInBuffer(r_blk, buf);
                if(r_now >= r_end) break;
