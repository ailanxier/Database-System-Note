...
> 数据库课程很难，知识琐碎，涉及科目广，PPT 讲的不明所以，谜语人化严重，部分老师只会念 PPT，建议有条件旁听张晓峰老师的课，体验会好上不少，实在听不懂只能去看 MOOC 的战老师了，但是估计效果也不是很好（PPT 朗诵大师）。因为从 PPT 获取的信息很有限（或者说信息非常劣质），笔者只能以它的基础上，在有限时间和能力上试图总结自己对数据库的理解，可能存在很多错误，需要自己加以判断。部分地方参考了『数据库系统概念（原书第六版）』，希望有所帮助，[这里放上我加了 OCR 和书签后书籍的阿里云盘链接](https://www.aliyundrive.com/s/6XK4ZGBBToB)。建议使用 Typora 查看，自定义主题为 [Purple]()
...
+ 函数依赖的特性：
  ...
  + 若 $X \rightarrow Y, Y \rightarrow X$，则记作 $X right\rightarrow Y$。
  + 若 $Y$ 不函数依赖于 $X$. 则记作 $X \not\rightarrow Y$。
...
+ 属性集闭包：
  ...
  ​	对 $  {R}(  {U},   {F}),   {X} \subseteq   {U},   {U}=\{  {A}_{1},  {~A}_{2}, \ldots,   {A}_{ {n}}\}$，令：$  {X}^{+}_{F}=\{  {A}_{  {i}} \mid. 用Armstrong ~公理可从 F 导出 .  {X} \rightarrow   {A}_{  {i}}\}$ 称 $  {X}^{+}{ }_{ {F}}$ 为 X 于 $F$ 的**属性集闭包**。
  显然 $X \subseteq X^{+}{ }_{F}$。
...
+ BCNF 分解 + 无损连接分解：
  ...
  + 无损连接分解成 BCNF：
    ...
    > $\quad$ 					 $result =(. result .-R_{i}) \cup(R_{i}-\beta) \cup(\alpha, \beta) $ （即用两个模式 $(R_{i}-\beta) \cup(\alpha, \beta)$ 取代原来的 $R_i$，由之前的快速判断的方法可知分解无损） 
...
+ 逻辑层优化：
  ...
  ​	优化关系代数操作执行次序，[后面会详细讲](#5.6.数据库查询优化技术)。
...
+ 关系的物理存储相关的参数：
  ...
  + $b$：每个**磁盘块的字节数**。
    $$
    \mathrm{B}_{\mathrm{RxS}}=\mathrm{T}_{\mathrm{R}} \mathrm{T}_{\mathrm{S}}(\mathrm{I}_{\mathrm{R}}+\mathrm{I}_{\mathrm{S}}) / \mathrm{b}
    $$
...
+ 案例分析：
  ...
  + $a$ 是 $R$ 的一个属性，在 $a$ 上有一个索引，并且考虑 $\sigma_{a=0}(R)$ 操作：
    ...
    + 如果 $R$ **不是聚簇**的，且**不使用索引**，查询代价 $=~20000$ 个 I/O 。
      ...
      > $V(A, R)$：$R$ 中属性 $A$ 出现不同值的数目，即 $\Pi_A(R)$ 的数目。
      >
      > PPT 中 $A,R$ 的顺序时常发生改变，不知道是笔误还是等价的。书上是属性在前，关系名在后。为了统一写法，下面全部采用书本的形式，因此和 PPT 几乎都是相反的。
...
+ 基于有序索引的连接算法（Zig-Zag 连接算法）:
  ...
  + $R$ 和 $S$ 均从左至右读取索引树的叶子结点：
    1. 读R的第一个索引项赋予 $a$，再读S的第一个索引项赋予 $b$；
    2. 如果 $a\not=b$，则：
...
+ 关系代数操作次序交换的等价性：
  ...
  + $L_6$，**选择和积**的**交换**律：
    ...
    $$
    + 若 ${F}={F}_{1} \wedge {F}_{2}, {~F}_{1}$ 只涉及 $\mathrm{E}_{1}$ 中属性，而 ${F}_{2}$ 涉及 ${E}_{1},{E}_{2}$ 中属性，则有：
  ...
  + $L_{10}$，**投影和并**的**交换**律：
    ...
    > $$
    > 一个大概的例子：假设 $E_1$ 只有 $3$ 个元组，在属性 ${A_{1}, \cdots, A_{n}}$ 上都相等，$E_2$ 只有一个元组，与 $E_1$ 的 $3$ 个元组之一相等。
    >
...
+ 统计信息：
  ...
  + $T_R$ 或 $T(R)$：关系 $R$ 的元组数目。

  + $V(A, R)$：$R$ 中属性 $A$ 出现不同值的数目，即 $\Pi_A(R)$ 的数目。

    > PPT 中 $A,R$ 的顺序时常发生改变，不知道是笔误还是啥情况。书上是属性在前，关系名在后。为了统一写法，下面全部采用书本的形式，因此和 PPT 几乎都是相反的。

+ 估计的目标：

  ​	给定一个表达式 $E$，如何估计 $E$ 的元组数目 $T(E)$。

+ 估算 $\pi_{\mathrm{A}}(\mathrm{R})$ 的大小：

  >PPT：$\mathrm{T}(\pi_{\mathrm{A}}(\mathrm{R}))=\mathrm{T}(\mathrm{R})$
  >
  >笔者认为应该是：$\mathrm{T}(\pi_{\mathrm{A}}(\mathrm{R}))=V(A,R)$
  >
  >投影运算并末减少行数，但可能有效地减少了存储结果关系的**块数**（每个元组所占的大小减少）。

+ 估算选择运算 $S= \sigma_{A=c}(R)$ 的大小：

  > 严谨：$\mathrm{T}(\mathrm{S})$ 介于 $[0,\mathrm{T}(\mathrm{R})-\mathrm{V}(\mathrm{A}, \mathrm{R})+1]$ 之间，原因分析：
  >
  > + 最小值为 $0$，因为可能关系 $R$ 中的 $A$ 属性不存在值为 $c$ 的元组。
  > + 最大值为 $\mathrm{T}(\mathrm{R})-\mathrm{V}(\mathrm{A}, \mathrm{R})+1$，即关系 $R$ 中的 $A$ 属性值不为 $c$ 的元组数目都是 $1$，共有 $V(A,R)$。
  >
  > 估计：$\mathrm{T}(\mathrm{S})=\mathrm{T}(\mathrm{R}) / \mathrm{V}(\mathrm{A}, \mathrm{R})$，即 $A$ 属性不同值的元组数相等。
  >
  > 暴力估计：不知道 $V(R,A)$ 时，默认为 $10$：$T(S) = T(R)/10$。

+ 估算选择运算 $\mathrm{S}=\sigma_{\mathrm{A}<\mathrm{c}}({R})$ 的大小：

  > 严谨：$\mathrm{T}(\mathrm{S})$ 介于 $0$ to $\mathrm{T}(\mathrm{R})$ 之间，原因：
  >
  > + 最小值：不存在这样的元组。
  > + 最多：所有元组都满足条件。
  >
  > 估计：$\mathrm{T}(\mathrm{S})=\mathrm{T}(\mathrm{R}) / 2$，应有一半的元组满足条件。
  >
  > 实际常用估计: $\mathrm{T}(\mathrm{S})=\mathrm{T}(\mathrm{R}) / 3$

+ 估算选择运算 ${S}=\sigma_{{A}=10 \mathrm{~AND~} B<20}({R})$ 的大小

  > 估计：$\mathrm{T}(\mathbf{S})=\mathrm{T}(\mathrm{R}) /(\mathbf{V}(\mathrm{R}, \mathbf{A})^{*} 3)$，原因：
  >
  > + $\sigma_{{A=10} \operatorname{AND} B<20}(R)=\sigma_{B<20}(\sigma_{A=10}(R))$
  > + $A=10$，得出 $T(S)=T(R) / V(R, A)$
  > + ${B}<20$，得出 $\mathrm{T}(\mathrm{S})=\mathrm{T}(\mathrm{S}) / 3$

+ 估算选择运算 $S = \sigma_{C_1~ or ~C_2}(R)$ 的大小：

  > 估计：$\begin{aligned}\mathrm{T}(\mathrm{S})=\mathrm{n}(1-(1-\frac{\mathrm{m}_{1}}{\mathrm{n}})(1-\frac{\mathrm{m}_{2}}{\mathrm{n}}))\end{aligned}$，分析：
  >
