...
+ 闭包：
  ...
  > - 增补律：
  >
  >   ​	若 $\alpha \rightarrow \beta$ 成立且 $\gamma$ 为一属性集，则 $\gamma \alpha \rightarrow \gamma \beta$ 成立。
  >
  > - 传递律：
  >
...
+ 数据库 - 表所占磁盘块的分配方法：
  ...
  + 按簇分配: 
    ...
    ​	按簇分配，簇之间靠指针连接，簇有时也称**片段**，结合前面两者优点。
...
+ 整个关系的二元操作：
  ...
  + 集合（包：允许重复）上的操作: 
    ...
    ​	$\cup_{\mathrm{S}}, \cap_{\mathrm{S}},-\mathrm{S}$
